...
void rewriteUpdateDppI32Call(CallInst *CI, Value *LaneId,
  ...
  //
  // Cross-widen row/bank gating: under modulo-replication the DPP
  // row_mask / bank_mask predicates the source kernel wrote refer to
  // source-wave-local row/bank, not target-wave row/bank.  For
  // wave32 source -> wave64 target, target lanes 0..31 are source-
  // wave-0 (rows 0..1), target lanes 32..63 are source-wave-1 (also
  // rows 0..1 in source view).  Computing RowIdx / BankIdx off the
  // raw target LaneId would gate target lane 48 (source-wave-1 lane
  // 16, source-row 1) against row_mask bit 3 instead of bit 1.  Mask
  // LaneId down to the source-wave-local lane first, then derive
  // row/bank from that.  When source == target (no cross-widen), the
  // mask is a no-op (LaneId already lives in [0, SourceWaveSize)).
  //
  // RowBase remains target-wave absolute: the bpermute selector
  // indexes the full target wave, and a source-wave-1 lane reading
  // its own source-wave neighbour must land in the target-wave
  // 16-lane bucket the selector points into.  `WithinRow = LaneId &
  // 0xF` is target-and-source consistent because source-wave-size is
  // a multiple of 16 and the low 4 bits coincide.
    ...
    if (!isDppCtrlRewritable(Ctrl)) {
      ...
      Os << "function '" << F.getName()
         ...
         << ". The cross-widen rewrite only covers quad_perm, "
            ...
            "See hotswap/docs/wave-size-translation.md section 5.3.";
