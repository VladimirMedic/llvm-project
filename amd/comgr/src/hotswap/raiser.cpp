...
// Categorised HotswapError subclasses + makeHotswap*Error factories are in hotswap-error.h/cpp

// ============================================================================
// Main raising function
// ============================================================================

static llvm::Expected<RaiseResult> raiseToIRImpl(
  ...
  if (SourceIsa.empty() ||
    ...
    return makeHotswapBadInputError(Twine("source ISA '") + SourceIsa +
  ...
  if (Error E = initMCState(Mc, SourceCpu)) {
    return makeHotswapBadInputError(toString(std::move(E)));
  }
  ...
  if (!TargetCpu.empty()) {
    ...
    if (!StiOrErr) {
      return makeHotswapBadInputError(toString(StiOrErr.takeError()));
    }
  ...
  unsigned ClassifierWaveIdLiftScalarizedSites = 0;
  {
    ...
    if (Report.hasUnrewritable() || Report.hasPendingRewrite()) {
      // Categorised wave-size-obstruction error; the factory names the
      // class in `Format`. Surface the full trace in `Detail` so
      // diagnostics can carry the per-site context forward without
      // re-invoking the classifier. Round-trip through handleAllErrors
      // to extract the structured fields, augment the detail with the
      // trace, then rebuild the error to propagate forward (the
      // specific subclass type is lost across the round-trip but the
      // stable Format string the lit tests grep on is preserved).
      llvm::Error Err = selectFailureFromReport(Report);
      std::string FormatStr;
      std::string MnemonicStr;
      std::string DetailStr;
      uint64_t OffsetVal = 0;
      llvm::handleAllErrors(std::move(Err), [&](const HotswapError &HE) {
        FormatStr = HE.Format;
      ...
      errs() << "transpiler: pre-translation abort: " << FormatStr
             << " on '" << MnemonicStr << "' at offset "
             << format_hex(OffsetVal, 1) << " -- "
             << (Report.firstUnrewritable()
                     ? "no rewrite in wave-size-translation.md "
                       "section 7's unrewritable table"
                     : "rewrite pending (wave-size-translation.md "
                       "section 7's pending-rewrite table)")
             << "\n"
             << Trace;
  ...
  for (const DecodedInst &Di : Insts) {
    ...
    return makeHotswapSPEUnsafeExecWriterError(Di);
  }
  ...
  if (!Tm) {
    ...
    return makeHotswapTargetMachineError();
  }
  ...
  if (!UserSgprLayout::tryFromKernelMeta(Meta, Isa, SourceIsa, UserSgprLayout,
    ...
    return Meta.HasKernelDescriptor
               ? makeHotswapUserSgprLayoutMismatchError(KernelName,
                                                        UserSgprFailureDetail)
               : makeHotswapMissingKernelDescriptorError(KernelName);
  }
  ...
  for (size_t InstIdx = 0; InstIdx < Insts.size(); ++InstIdx) {
    ...
    if (bool(Ctx.PendingFailure)) {
      llvm::Error E = std::move(Ctx.PendingFailure);
      Ctx.PendingFailure = llvm::Error::success();
      return std::move(E);
    }
    ...
    // specific shape (Hr.Failure carries a HotswapError), or no handler
    // claimed it at all -- promote to `UnsupportedOpcode` and bucket
    // by format.
    if (Hr.Failure) {
      return std::move(Hr.Failure);
    }
    // `formatName` returns Error iff the disassembler produced an
    // opcode whose TSFlags don't match any known AMDGPU encoding
    // family -- fold that into the bucket label as `<unknown>` rather
    // than failing the kernel raise for diagnostic reasons.
    Expected<StringRef> FmtOrErr = formatName(Di.TsFlags, Di.Inst.getOpcode());
    StringRef FmtName;
    if (FmtOrErr) {
      FmtName = *FmtOrErr;
    } else {
    ...
    return makeHotswapUnsupportedOpcodeError(Di, FmtName);
  }
  ...
  if (EnableWritelaneRewrite) {
    ...
    if (RewriteReport.refusedSgprForced()) {
      ...
      if (!ForceThreadLoopProjection &&
        ...
        if (threadLoopUnsupportedWorkgroupMemoryOrBarrier(
          ...
          errs() << "transpiler: thread-loop fallback not eligible for kernel '"
                 ...
                 << "\n";
          return renderAndRebuildHotswapError(
              makeHotswapCrossWaveRewriteOracleDisagreementError(
                  KernelName, ThreadLoopUnsupportedDetail),
              [&](StringRef Format, StringRef Mnemonic, uint64_t,
                  StringRef Detail) {
                errs() << "transpiler: post-raise abort: " << Format
                       << " on '" << Mnemonic << "' -- " << Detail << "\n";
              });
        }
      ...
      return renderAndRebuildHotswapError(
          ...
          [&](StringRef Format, StringRef Mnemonic, uint64_t,
              StringRef Detail) {
            errs() << "transpiler: post-raise abort: " << Format << " on '"
                   << Mnemonic << "' -- " << Detail << "\n";
          });
    }
    ...
    if (ClassifierWaveIdLiftScalarizedSites > 0 &&
      ...
      return renderAndRebuildHotswapError(
          makeHotswapCrossWaveRewriteOracleDisagreementError(KernelName,
                                                              Os.str()),
          [&](StringRef Format, StringRef Mnemonic, uint64_t,
              StringRef Detail) {
            errs() << "transpiler: post-raise abort: " << Format << " on '"
  ...
  // `ObstructionKind::WorkitemIdPredicateChain`.
  {
    ...
    if (PredReport.Refused) {
      ...
      bool PhantomSubcase = PredReport.WaveNativePhantomRefusal;
      return renderAndRebuildHotswapError(
          makeHotswapCrossWavePredicateChainError(KernelName,
                                                   PredReport.RefusalDetail),
          [&](StringRef Format, StringRef Mnemonic, uint64_t,
              StringRef Detail) {
            errs() << "transpiler: pre-translation abort: " << Format
                   << " on '" << Mnemonic << "' -- " << Detail << "\n";
            errs() << "  outcome: (c) refuse -- "
                      "WorkitemIdPredicateChain (section 3 Class 5"
                   << (PhantomSubcase ? " phantom-lane sub-case" : "")
                   << ")\n";
          });
    }
  }
  ...
  if (moduleUsesTDMRuntime(M)) {
    if (!linkTDMRuntime(M, CompilationTargetIsa)) {
      ...
      return makeHotswapIRVerificationError("TDM runtime bitcode link failed");
    }
  }
  ...
  if (verifyModule(M, &VerifyOs)) {
    ...
    return makeHotswapIRVerificationError(VerifyErr);
  }
