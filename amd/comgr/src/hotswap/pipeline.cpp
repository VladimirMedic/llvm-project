...
static bool raiseAndCompileKernel(const TextSection &text,
  ...
  llvm::Expected<RaiseResult> raisedOrErr = raiseToIR(
      text.Bytes, sourceISA, kernelName, meta, kernelOffset, targetISA,
      options.EnableWritelaneRewrite, options.EnableWaveNative);
  if (!raisedOrErr) {
    llvm::errs() << "transpiler: Raising '" << kernelName << "' to LLVM IR failed";
    result.FailKernel = kernelName;
    // Extract structured fields off the HotswapError payload to
    // populate the pipeline-level diagnostic record. The Format
    // string matches the former `reasonString(RaiseFailureReason)`
    // output character-for-character (preserved across the
    // migration so lit-test FileChecks keep matching).
    llvm::handleAllErrors(
        raisedOrErr.takeError(), [&](const HotswapError &HE) {
          if (!HE.Mnemonic.empty()) {
            llvm::errs() << " (unsupported: " << HE.Mnemonic << ")";
            result.FailMnemonic = HE.Mnemonic;
          }
          result.FailReason = HE.Format;
          result.FailFormat = HE.Format;
          result.FailDetail = HE.Detail;
          result.FailOffset = HE.Offset;
        });
    llvm::errs() << "\n";
    result.Timings.raiseSeconds += timingElapsed(options.CollectTimings, raiseStart);
    return false;
