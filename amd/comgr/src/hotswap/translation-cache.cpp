...
KeyData buildKeyData(const TranslationCacheRequest &request,
  ...
  // listKernelNames returns Expected<SmallVector<string>>; unwrap and
  // surface the error rather than silently coercing through .begin().
  // The translation-cache layer treats a missing AMDGPU metadata note
  // (the only HotswapError condition listKernelNames raises) as a
  // pre-condition for downstream identity construction, so we
  // populate `data.error` and leave `kernelNames` empty so callers
  // detect the broken cache material via the `error` field.
  ...
  if (namesOrErr) {
    ...
    data.error = "listKernelNames failed: " +
