...
HandlerResult handleSMEM(RaiseContext &Ctx, const DecodedInst &Di,
  ...
  if (Sop == CanonicalOp::S_LOAD_B32 || Sop == CanonicalOp::S_LOAD_B64 ||
    ...
    if (BaseIsKernargPair && ImmOffset &&
      ...
      if (isStrictMode()) {
        Hr.Failure = makeHotswapStrictUnsafeLoweringError(
            Di, "implicitarg.ptr",
      ...
      if (!HiddenBase.Value) {
        Hr.Failure =
            makeHotswapUnsupportedShapeError(Di, "SMEM", HiddenBase.FailureDetail);
        return Hr;
      }
      for (int D = 0; D < LoadDwords; D++) {
        ...
        if (!Dw.Matched || !Dw.Value) {
          Hr.Failure = makeHotswapUnsupportedShapeError(
              Di, "SMEM",
    ...
    if (Di.isImm(OffIdx)) {
      ...
      if (BaseIsKernargPair && Ctx.Kernargs.ImplicitArgsBase > 0 &&
        ...
        if (Hidden.Matched) {
          if (!Hidden.Value) {
            Hr.Failure =
                makeHotswapUnsupportedShapeError(Di, "SMEM", Hidden.FailureDetail);
            return Hr;
          }
