...
int main(int argc, char **argv) {
  ...
  if (emitIr) {
    ...
    if (!raisedOrErr) {
      ...
      // success path, so we cannot dump partial IR here. Callers that
      // need stderr diagnostics (abort-gate lit tests, etc.) FileCheck
      // the raiser's stderr -- we leave that untouched.
      llvm::handleAllErrors(
          raisedOrErr.takeError(),
          [&](const COMGR::hotswap::HotswapError &HE) {
            llvm::errs()
                << "raise_cli: kernel '" << target << "' failed to raise: "
                << (HE.Mnemonic.empty() ? "unknown" : HE.Mnemonic.c_str())
                << " ["
                << (HE.Format.empty() ? "unknown" : HE.Format.c_str())
                << "] @offset=0x";
            llvm::errs().write_hex(HE.Offset);
            if (!HE.Detail.empty())
              llvm::errs() << " :: " << HE.Detail;
            llvm::errs() << "\n";
          });
    ...
    llvm::outs().write(raisedOrErr->IrText.data(), raisedOrErr->IrText.size());
    return 0;
  }
  ...
  for (auto &kName : kernelNames) {
    ...
    if (pid == 0) {
      ...
      if (raisedOrErr) {
        shm->lifted = raisedOrErr->LiftedCount;
        shm->total = raisedOrErr->TotalCount;
      } else {
        // Partial-progress counts are not available with the
        // Expected<>-shaped API; raise_cli's fork-mode shm record
        // reports 0/0 on failure (matches the kerneldex runner's
        // expectation for a crashed-or-refused row).
        shm->lifted = 0;
        shm->total = 0;
        llvm::handleAllErrors(
            raisedOrErr.takeError(),
